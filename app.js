...
function renderD(d){
  ...
  else if(d.t==='scene_hatch'){
    // Zone hachurée triangulaire d'entrée/sortie voie
    const x1=d.x1,y1=d.y1,x2=d.x2,y2=d.y2;
    ctx.save();
    ctx.fillStyle='rgba(200,195,185,.7)';
    ctx.beginPath();ctx.moveTo(x1,y1);ctx.lineTo(x2,y1);ctx.lineTo(x2,y2);ctx.closePath();ctx.fill();
    // Hachures blanches diagonales
    ctx.strokeStyle='rgba(255,255,255,.7)';ctx.lineWidth=3;ctx.setLineDash([8,8]);
    const W2=x2-x1, H2=y2-y1, step=18;
    ctx.beginPath();
    for(let d2=0;d2<W2+H2;d2+=step){
      const ax=x1+Math.min(d2,W2), ay=y1+Math.max(0,d2-W2);
      const bx=x1+Math.max(0,d2-H2), by=y1+Math.min(d2,H2);
      ctx.moveTo(ax,ay);ctx.lineTo(bx,by);
    }
    ctx.clip();
    ctx.beginPath();ctx.moveTo(x1,y1);ctx.lineTo(x2,y1);ctx.lineTo(x2,y2);ctx.closePath();ctx.stroke();
    ctx.setLineDash([]);
    ctx.restore();
  }
...
function drawCarSchema(cx,cy){
  ...
  // Lunette arrière inclinée (hayon)
  ctx.beginPath();ctx.moveTo(82,-54);ctx.lineTo(165,-12);ctx.lineTo(158,-4);ctx.lineTo(78,-4);ctx.closePath();ctx.fill();ctx.stroke();
  // Indicateur 'ARRIÈRE' sous le hayon
  ctx.fillStyle='#0a7aff';ctx.font='bold 9px sans-serif';ctx.textAlign='center';ctx.textBaseline='top';
  ctx.fillText('hayon',128,4);

  // Phares AVANT gauche (jaune) – le capot est à gauche
  ctx.fillStyle='#ffee88';ctx.strokeStyle='#cc9900';ctx.lineWidth=1.5;
  ctx.beginPath();ctx.ellipse(-242,10,10,13,0.3,0,Math.PI*2);ctx.fill();ctx.stroke();
  // Feux ARRIÈRE droite (rouge) – le hayon est à droite
  ctx.fillStyle='#dd3333';ctx.strokeStyle='#990000';ctx.lineWidth=1.5;
  ctx.beginPath();ctx.ellipse(222,10,10,13,-0.3,0,Math.PI*2);ctx.fill();ctx.stroke();

  // Labels AVANT / ARRIÈRE
  ctx.fillStyle='#e05c00';ctx.font='bold 12px sans-serif';ctx.textAlign='center';ctx.textBaseline='bottom';
  // AVANT = gauche (phares jaunes), ARRIÈRE = droite (feux rouges)
  ctx.font='bold 13px sans-serif'; ctx.textAlign='center'; ctx.textBaseline='bottom';
  ctx.fillStyle='#e05c00'; ctx.fillText('◀ AVANT',-185,-80);
  ctx.fillStyle='#0a7aff'; ctx.fillText('ARRIÈRE ▶',185,-80);
...
function drawClutch3(cx,cy){
  ...
  // ── Paramètres globaux ────────────────────────────────────────────────
  const rows = [
    { state:'debraye',  col:'#c0392b', label:'Position débrayée',          pedLabel:'Pied gauche à fond\nsur la pédale.', wheelTurns:false },
    { state:'patinage', col:'#2c3e50', label:'position embrayée\nPoint mort',pedLabel:'Pied gauche relevé.',              wheelTurns:false },
    { state:'embraye',  col:'#2c3e50', label:'Position embrayée\n1ère enclenchée', pedLabel:'Pied gauche relevé.',        wheelTurns:true  },
  ];
  const nRows = rows.length;
  const rowH  = H / nRows;

  // ── Dimensions communes (en px canvas) ────────────────────────────────
  // On dessine à l'échelle du canvas directement (pas de scale()) pour
  // que les traits soient nets quelle que soit la taille de l'écran.
  const margin   = W * 0.04;          // marge gauche/droite
  const drawW    = W - margin * 2;    // largeur utile
  // Colonnes : [moteur] [2 disques] [boîte] [arbre+roue] [pied]
  const colMot   = margin + drawW * 0.10;
  const colDisk  = margin + drawW * 0.30;
  const colBVT   = margin + drawW * 0.50;
  const colRoue  = margin + drawW * 0.68;
  const colPied  = margin + drawW * 0.88;
  const boxW     = Math.min(70, drawW * 0.12);
  const boxH     = Math.min(50, rowH  * 0.38);
  const diskR    = Math.min(28, rowH  * 0.22);
  const shaftY2  = 5; // demi-épaisseur arbre

  rows.forEach((row, i) => {
    const cy2   = i * rowH + rowH / 2;
    const col   = row.col;
    const gap   = row.state === 'debraye' ? diskR * 0.9
                : row.state === 'patinage' ? diskR * 0.4
                : 0; // écart entre disque moteur et disque boîte
    ...
    // ──────────────── MOTEUR ────────────────────────────────────────────
    // Rectangle rouge à bords arrondis
    ctx.fillStyle='rgba(180,40,20,.12)'; ctx.strokeStyle='#c0392b'; ctx.lineWidth=2;
    rr(ctx, colMot-boxW/2, cy2-boxH/2, boxW, boxH, 7); ctx.fill(); ctx.stroke();
    ctx.fillStyle='#c0392b'; ctx.font=`bold ${Math.min(11,boxW*.17)}px sans-serif`;
    ctx.textAlign='center'; ctx.textBaseline='middle';
    ctx.fillText('Moteur', colMot, cy2);

    // Arbre moteur → disque gauche
    const shG = ctx.createLinearGradient(0,cy2-shaftY2,0,cy2+shaftY2);
    shG.addColorStop(0,'#bbb'); shG.addColorStop(.5,'#eee'); shG.addColorStop(1,'#888');
    ctx.fillStyle=shG;
    ctx.fillRect(colMot+boxW/2, cy2-shaftY2, colDisk-diskR-colMot-boxW/2, shaftY2*2);

    // ──────────────── DISQUES ───────────────────────────────────────────
    // Disque gauche : relié au moteur (gris foncé, plein)
    const dxMot = colDisk - diskR * 0.5;
    ctx.fillStyle='rgba(50,50,50,.35)'; ctx.strokeStyle='#444'; ctx.lineWidth=2.5;
    ctx.beginPath(); ctx.arc(dxMot, cy2, diskR, 0, Math.PI*2); ctx.fill(); ctx.stroke();
    // Rayons stylisés
    ctx.strokeStyle='rgba(255,255,255,.35)'; ctx.lineWidth=1.5;
    for(let k=0;k<6;k++){
      const a=k*Math.PI/3;
      ctx.beginPath(); ctx.moveTo(dxMot,cy2); ctx.lineTo(dxMot+diskR*.8*Math.cos(a), cy2+diskR*.8*Math.sin(a)); ctx.stroke();
    }
    // Petit carré vert (boîte reliée au 2e disque) - fidèle à l'image
    const sqW=diskR*.7, sqH=diskR*.7;
    ctx.fillStyle='rgba(40,160,40,.3)'; ctx.strokeStyle='#30b94a'; ctx.lineWidth=2;
    ctx.fillRect(dxMot+diskR*1.0+gap-sqW/2, cy2-sqH/2, sqW, sqH);
    ctx.strokeRect(dxMot+diskR*1.0+gap-sqW/2, cy2-sqH/2, sqW, sqH);

    // Disque droit : relié à la boîte (coloré selon état, décalé si débrayé)
    const dxBVT = colDisk + diskR * 0.5 + gap;
    ctx.fillStyle = col+'28'; ctx.strokeStyle=col; ctx.lineWidth=2.5;
    ctx.beginPath(); ctx.arc(dxBVT, cy2, diskR, 0, Math.PI*2); ctx.fill(); ctx.stroke();
    for(let k=0;k<6;k++){
      const a=k*Math.PI/3+Math.PI/6;
      ctx.strokeStyle=col+'66'; ctx.lineWidth=1.5;
      ctx.beginPath(); ctx.moveTo(dxBVT,cy2); ctx.lineTo(dxBVT+diskR*.8*Math.cos(a), cy2+diskR*.8*Math.sin(a)); ctx.stroke();
    }

    // Écart visible si débrayé/patinage
    if(gap > 1){
      ctx.strokeStyle=col; ctx.lineWidth=1.5; ctx.setLineDash([4,3]);
      ctx.beginPath(); ctx.moveTo(dxMot+diskR, cy2-diskR*1.3); ctx.lineTo(dxBVT-diskR, cy2-diskR*1.3); ctx.stroke();
      ctx.setLineDash([]);
      // Flèches d'écart
      [[dxMot+diskR+2, 1],[ dxBVT-diskR-2, -1]].forEach(([x,dir])=>{
        ctx.fillStyle=col;
        ctx.beginPath(); ctx.moveTo(x, cy2-diskR*1.3);
        ctx.lineTo(x+dir*10, cy2-diskR*1.3-5); ctx.lineTo(x+dir*10, cy2-diskR*1.3+5);
        ctx.closePath(); ctx.fill();
      });
    }

    // Arbre boîte → roue
    const axStart = dxBVT + diskR;
    ctx.fillStyle=shG; ctx.fillRect(axStart, cy2-shaftY2, colRoue-diskR-axStart, shaftY2*2);

    // ──────────────── BOÎTE DE VITESSES ────────────────────────────────
    ctx.fillStyle='rgba(48,185,74,.12)'; ctx.strokeStyle='#30b94a'; ctx.lineWidth=2;
    rr(ctx, colBVT-boxW/2, cy2-boxH/2, boxW, boxH, 7); ctx.fill(); ctx.stroke();
    ctx.fillStyle='#30b94a'; ctx.font=`bold ${Math.min(10,boxW*.155)}px sans-serif`;
    ctx.textAlign='center'; ctx.textBaseline='middle';
    ctx.fillText('Boîte', colBVT, cy2-7);
    ctx.fillText('vitesses', colBVT, cy2+7);

    // ──────────────── ROUE ──────────────────────────────────────────────
    const rRoue = Math.min(diskR*1.3, rowH*0.38);
    // Pneu extérieur
    ctx.fillStyle='#222'; ctx.beginPath(); ctx.ellipse(colRoue, cy2, rRoue*.45, rRoue, 0, 0, Math.PI*2); ctx.fill();
    ctx.strokeStyle='#555'; ctx.lineWidth=2; ctx.beginPath(); ctx.ellipse(colRoue, cy2, rRoue*.45, rRoue, 0, 0, Math.PI*2); ctx.stroke();
    // Jante
    const rJ = rRoue*.7;
    ctx.fillStyle='#bbb'; ctx.beginPath(); ctx.ellipse(colRoue, cy2, rJ*.45, rJ, 0, 0, Math.PI*2); ctx.fill();
    // Rayons jante
    ctx.strokeStyle='#888'; ctx.lineWidth=2;
    for(let k=0;k<5;k++){
      const a=k*Math.PI*2/5;
      ctx.beginPath(); ctx.moveTo(colRoue, cy2); ctx.lineTo(colRoue+rJ*.4*Math.cos(a), cy2+rJ*.8*Math.sin(a)); ctx.stroke();
    }
    // Flèches de rotation si roues tournent
    if(row.wheelTurns){
      ctx.strokeStyle='#0a7aff'; ctx.lineWidth=2.5; ctx.lineCap='round';
      // Arc de rotation antihoraire en haut
      ctx.beginPath(); ctx.arc(colRoue, cy2, rRoue+8, -Math.PI*0.7, -Math.PI*0.1); ctx.stroke();
      ctx.fillStyle='#0a7aff';
      const ae=-Math.PI*0.1;
      ctx.beginPath();
      ctx.moveTo(colRoue+(rRoue+8)*Math.cos(ae), cy2+(rRoue+8)*Math.sin(ae));
      ctx.lineTo(colRoue+(rRoue+8)*Math.cos(ae)-8, cy2+(rRoue+8)*Math.sin(ae)-8);
      ctx.lineTo(colRoue+(rRoue+14)*Math.cos(ae+.2), cy2+(rRoue+14)*Math.sin(ae+.2));
      ctx.closePath(); ctx.fill();
    }
    // Label "Roues tournent" / "Roues ne tournent pas"
    ctx.fillStyle='#333'; ctx.font=`${Math.min(9,rowH*.07)}px sans-serif`;
    ctx.textAlign='center'; ctx.textBaseline='top';
    const wlbl = row.wheelTurns ? 'Roues tournent' : 'Roues ne\ntournent pas';
    wlbl.split('\n').forEach((ln,k)=>ctx.fillText(ln, colRoue, cy2+rRoue+4+k*11));

    // ──────────────── PIED ──────────────────────────────────────────────
    // L'image de référence montre :
    //  Débrayé  : jambe inclinée droite, pied pointant VERS LE BAS (pédale au bas)
    //  Patinage : jambe droite, pied mi-incliné
    //  Embrayé  : jambe droite, pied légèrement incliné (pédale haute, relevé)
    const pedX = colPied;
    const pedY = cy2;
    const lw2 = Math.min(5, rowH*0.038);
    ctx.strokeStyle='#444'; ctx.lineWidth=lw2; ctx.lineCap='round';

    // Pivot genou (point fixe en haut)
    const kneeX = pedX - rowH*0.04;
    const kneeY = pedY - rowH*0.28;

    // Position cheville selon état (plus basse = pédale enfoncée)
    let ankleX, ankleY, toeX, toeY;
    if(row.state==='debraye'){
      // Pied à fond : cheville basse et à gauche, orteil vers le bas
      ankleX = pedX - rowH*0.10;
      ankleY = pedY + rowH*0.08;
      toeX   = ankleX - rowH*0.18;
      toeY   = ankleY + rowH*0.05;
    } else if(row.state==='patinage'){
      // Mi-course : position intermédiaire
      ankleX = pedX - rowH*0.06;
      ankleY = pedY - rowH*0.02;
      toeX   = ankleX - rowH*0.16;
      toeY   = ankleY + rowH*0.08;
    } else {
      // Pied relevé : cheville haute, orteil vers le bas-gauche
      ankleX = pedX - rowH*0.04;
      ankleY = pedY - rowH*0.10;
      toeX   = ankleX - rowH*0.14;
      toeY   = ankleY + rowH*0.12;
    }

    // Jambe (genou → cheville)
    ctx.beginPath(); ctx.moveTo(kneeX, kneeY); ctx.lineTo(ankleX, ankleY); ctx.stroke();
    // Pied (cheville → orteil)
    ctx.lineWidth = Math.min(6, rowH*0.044);
    ctx.beginPath(); ctx.moveTo(ankleX, ankleY); ctx.lineTo(toeX, toeY); ctx.stroke();
    // Pédale (petite ligne au bout du pied)
    ctx.strokeStyle='#777'; ctx.lineWidth=Math.min(4,rowH*.03);
    const pLen = rowH*0.09;
    ctx.beginPath();
    ctx.moveTo(toeX - pLen*0.4, toeY - pLen*0.3);
    ctx.lineTo(toeX + pLen*0.4, toeY + pLen*0.3);
    ctx.stroke();

    // Flèche bleue si pied enfoncé (indique la pression)
    if(row.state==='debraye'){
      ctx.strokeStyle='#0a7aff'; ctx.lineWidth=2;
      const ax=toeX+rowH*0.03, ay1=toeY-rowH*0.15, ay2=toeY-rowH*0.01;
      ctx.beginPath(); ctx.moveTo(ax,ay1); ctx.lineTo(ax,ay2); ctx.stroke();
      ctx.fillStyle='#0a7aff';
      ctx.beginPath(); ctx.moveTo(ax,toeY); ctx.lineTo(ax-5,toeY-9); ctx.lineTo(ax+5,toeY-9); ctx.closePath(); ctx.fill();
    }

    // Label pied
    ctx.fillStyle='#555'; ctx.font=`${Math.min(10,rowH*.075)}px sans-serif`;
    ctx.textAlign='center'; ctx.textBaseline='top';
    row.pedLabel.split('\n').forEach((ln,k) => ctx.fillText(ln, pedX, cy2+rowH*0.38+k*12));

    // ──────────────── LABEL ÉTAT (à gauche) ─────────────────────────────
    ctx.fillStyle = row.state==='debraye' ? '#c0392b' : '#2c3e50';
    ctx.font=`bold ${Math.min(12,rowH*.088)}px sans-serif`;
    ctx.textAlign='left'; ctx.textBaseline='middle';
    const lx2 = margin;
    row.label.split('\n').forEach((ln,k,arr)=>{
      ctx.fillText(ln, lx2, cy2 + (k-(arr.length-1)/2)*14);
    });
  });

  // ── Titre ──────────────────────────────────────────────────────────────
  ctx.fillStyle='rgba(0,0,0,.07)'; ctx.fillRect(0,0,W,44);
  ctx.strokeStyle='rgba(0,0,0,.12)'; ctx.lineWidth=1;
  ctx.beginPath(); ctx.moveTo(0,44); ctx.lineTo(W,44); ctx.stroke();
  ctx.fillStyle='#1a1a1c'; ctx.font=`bold ${Math.min(15,W*.02)}px sans-serif`;
  ctx.textAlign='center'; ctx.textBaseline='middle';
  ctx.fillText("Fonctionnement de l'embrayage — 3 positions", cx, 22);
}
...
function loadScene(name){
  ...
  // ── VOIE D'INSERTION ─────────────────────────────────────────────────
  // Référence image : route horizontale 2x2, sens gauche→droite.
  // La voie d'insertion arrive par le BAS-GAUCHE et rejoint la voie
  // de DROITE (voie lente) au 2/3 de l'écran. Zone hachurée triangulaire
  // entre la voie principale et l'entrée de la voie d'insertion.

  // ── VOIE DE DÉCÉLÉRATION ──────────────────────────────────────────────
  // Référence image : route 2x2, sens gauche→droite.
  // La voie de décélération se DÉTACHE vers le bas-droit à partir de la
  // voie lente (droite). Zone hachurée triangulaire à la séparation.
  // Flèches vertes montrant le flux principal et la sortie.
